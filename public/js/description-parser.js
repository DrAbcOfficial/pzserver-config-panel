...
function makeImageUrl(subModPath, imagePath) {
  const safe = (imagePath || "").trim();
  if (!safe) return "";
  const path = (subModPath ? subModPath.replace(/\\/g, "/") + "/" : "") + safe.replace(/\\/g, "/");
  return "/api/workshop-poster?rel=" + encodeURIComponent(path);
}
...
function handleFormatTag(tagBody, state) {
  switch (tagBody) {
    case "H1":
...
function handleValueTag(tagBody, state) {
  const tagLC = tagBody.toLowerCase();

  if (tagLC.startsWith("rgb:")) {
    return { color: tagBody.slice(4) };
  }

  if (tagLC.startsWith("pushrgb:")) {
    return {
      color: tagBody.slice(8),
...
function emitFormatTag(tagBody, state) {
...
function emitStructuralTag(tagBody, subModPath) {
  switch (tagBody) {
    case "LINE":
      return "<br>";
    case "BR":
  ...
  if (imgMatch) {
    ...
    const url = makeImageUrl(subModPath, imgPath);
    if (url) {
  ...
  if (vcMatch) {
    const parts = vcMatch[1].split(",").map(function (s) { return s.trim(); });
    const videoName = parts[0] ?? "";
    const backup = parts[3] && !parts[4] ? parts[3] : null;
    if (backup) {
      const url = makeImageUrl(subModPath, backup);
      if (url) {
...
export function parseDescriptionTags(description, subModPath) {
  ...
  while (match !== null) {
    ...
    result += emitStructuralTag(tagBody, subModPath);
